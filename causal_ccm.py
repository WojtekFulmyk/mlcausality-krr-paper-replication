...
class ccm:
    ...
    def predict(self, t):
        ...
        # Fix FloatingPointError: underflow encountered in exp
        u = np.exp(np.clip( -nearest_distances/np.max([eps, nearest_distances[0]]), -500, 500 )) # we divide by the closest distance to scale
        w = u / np.sum(u)
...
def loop_causal_cmm(data, tau=1, E=2, L=None):
    results_list = []
    permute_list = list(itertools.permutations(range(data.shape[1]),2))
    for X_idx, y_idx in permute_list:
        ccm1 = ccm(X=data[:,X_idx], Y=data[:,y_idx], tau=tau, E=E, L=L)
        score, pval = ccm1.causality()
        results_list.append([X_idx,y_idx,score,pval])
    out_df = pd.DataFrame(results_list, columns=['X','y','score','pvalue'])
    return out_df
